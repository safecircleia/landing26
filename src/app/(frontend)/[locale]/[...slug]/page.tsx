...
export async function generateStaticParams() {
  ...
  // Get pages for the default locale to generate static params
  const pages = await payload.find({
    collection: 'pages',
    depth: 0,
    limit: 300,
    locale: 'en', // Use default locale for static generation
    select: {
      breadcrumbs: true,
    },
    where: {
...
const queryPage = cache(
  async ({ slug, locale }: { locale: TypedLocale; slug: string[] }): Promise<null | PageType> => {
    ...
    // Handle home page case when slug is empty
    const slugSegments = slug && slug.length > 0 ? slug : ['home']
    ...
    // If we only have one result and it's what we're looking for, return it
    if (result.docs.length === 1) {
      return result.docs[0]
    }

    // Otherwise, try to match by breadcrumbs
    const pagePath = slug && slug.length > 0 ? `/${slug.join('/')}` : '/home'

    const page = result.docs.find(({ breadcrumbs }: PageType) => {
      if (!breadcrumbs || breadcrumbs.length === 0) {
        // For pages without breadcrumbs, check if this is the home page
